...
a. Recreate the R code necessary to generate the following graphs. You will need to create 8 separate plots for this exercise. Note that wherever a categorical variable is used in the plot, it's `metro`.
...
# use patchwork to combine together for saving to disk
library(patchwork)
theme_set(theme_grey(base_size = 11))

# plot A
{
  ggplot(data = county, mapping = aes(x = homeownership, y = poverty)) +
    geom_point() +
    ggtitle("Plot A")
} +
  # plot B
  ...
  # plot C
  {
    ggplot(data = county, mapping = aes(x = homeownership, y = poverty)) +
      geom_point() +
      geom_smooth(mapping = aes(group = metro), se = FALSE, color = "green") +
      ggtitle("Plot C")
  } +
  # plot D
  {
  ...
  {
    ggplot(data = county, mapping = aes(x = homeownership, y = poverty)) +
      geom_point(mapping = aes(color = metro)) +
      geom_smooth(mapping = aes(linetype = metro), se = FALSE) +
      ggtitle("Plot E")
  } +
  # plot F
  {
    ggplot(data = county, mapping = aes(
      x = homeownership, y = poverty,
      color = metro
    )) +
      geom_point() +
      geom_smooth(se = FALSE) +
